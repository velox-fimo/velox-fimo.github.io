...
function updateBadge() { const total = getCart().length; document.querySelectorAll("#cart-badge").forEach(b => b.textContent = total); const count = document.getElementById("cart-count"); if (count) count.textContent = total; }
...
document.getElementById("search-btn").addEventListener("click", filterDealers);
document.getElementById("city-input").addEventListener("keydown", (e) => { if (e.key === "Enter") filterDealers(); });

function filterDealers() {
  const query = document.getElementById("city-input").value.trim().toLowerCase();
  const cards = document.querySelectorAll(".dealer-card");
  if (!query) { cards.forEach(c => c.classList.remove("hidden")); return; }
  cards.forEach(c => {
    const city = c.dataset.city || "";
    const text = c.innerText.toLowerCase();
    if (city.includes(query) || text.includes(query)) c.classList.remove("hidden");
